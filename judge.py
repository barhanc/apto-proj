...
def readSolution(fname):
    ...
    for i in range(M):
        ...
        if X < 0 or Y < 0 or L < 0:
            raise Exception(f"Liczba ujemna w rozwiazaniu, linia {num}: {line}")
        if D not in "UDLR":
            raise Exception(f"Bledny kierunek w rozwiazaniu, linia {num}: {line}")
        sol.append((X, Y, D, L))
...
def singleMove(board, sx, sy, ex, ey, dx, dy, move_num):
    if (dx == 1 or dy == 1) and getboard(board, ex + dx, ey + dy) != ".":
        raise Exception(f"Bledny ruch {move_num}")
    if (dx == -1 or dy == -1) and getboard(board, sx + dx, sy + dy) != ".":
        raise Exception(f"Bledny ruch {move_num}")
...
def makeMove(board, W, H, move, move_num):
    ...
    dx, dy = DIR[d]
    for s in range(l):
        if (sx + dx < 0 or sy + dy < 0 or ex + dx >= W or ey + dy >= H) and dean:
            return True
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        try:
            ...
            if len(steps) > N:
                raise Exception("za duzo krokow w rozwiazaniu")
